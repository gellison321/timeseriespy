...
def dtw_matrix(I, J, w = None, r = np.inf):
...
def dtw_path(matrix):
    '''
    Computes the path of the dynamic time warping algorithm, given a dtw cost matrix.
    ...
    while i != 0 and j != 0:
        path.append((i - 1, j - 1))
        min_index = utils['argmin']([matrix[i-1, j-1], matrix[i-1, j], matrix[i, j-1]])
        if min_index == 0:
            i -= 1
            j -= 1
        elif min_index == 1:
